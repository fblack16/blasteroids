...
int main(int argc, char** argv) {
    ...
    while (running) {
        ...
        switch (event.type) {
            case ALLEGRO_EVENT_TIMER:
                if (event.timer.source == game_state.timer_ptr) {
                    ...
                    update_asteroid_containers(game_state.asteroid_containers, (float)frame_delta);
                    //fire_blast(&game_state);
                    update_blast_containers(game_state.blast_containers, (float)frame_delta);
                    fire_blast(&game_state);
        ...
        if (needs_redraw && al_event_queue_is_empty(game_state.event_queue_ptr)) {
            ...
            draw_asteroid_containers(game_state.asteroid_containers);
            draw_blast_containers(game_state.blast_containers);
            al_flip_display();
            needs_redraw = false;
        }
